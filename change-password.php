...
if(isset($_POST['changepwd']))
{
  ...
  $sql="SELECT password FROM userregistration where password=:op";
  ...
  if($row_cnt > 0)
  {
    $con="update userregistration set password=:np, passUdateDate=:udate where id=:ai";
...
<?php $result ="SELECT passUdateDate FROM userregistration WHERE id=:ai";
$stmt = oci_parse($dbconn, $result);
oci_bind_by_name($stmt, ":ai", $ai);
oci_execute($stmt);
$result = oci_fetch_assoc($stmt);
?>

                                    Last Updation Date:&nbsp;<?php echo $result['PASSUDATEDATE']; ?> </div>
