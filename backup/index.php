...
if(isset($_POST['login']))
{
    ...
    $query = "SELECT email, password, id FROM userregistration WHERE (email = :emailreg OR regNo = :emailreg) AND password = :password";
    $stm = oci_parse($dbconn, $query);
    oci_bind_by_name($stm, ":emailreg", $emailreg);
    ...
    if($row)
    {
        ...
        oci_bind_by_name($stm, ":uid", $_SESSION['id']);
        oci_bind_by_name($stm, ":uemail", $_SESSION['login']);
        oci_bind_by_name($stm, ":ip", $ip);
        oci_bind_by_name($stm, ":city", $city);
        oci_bind_by_name($stm, ":country", $country);
        oci_execute($stm);
...
if (isset($_POST['login'])) {
   ...
   if($row){
		...
		//getting the email
		//$aid99 = $_SESSION['id'];
		//$query99 = "SELECT EMAIL FROM userregistration WHERE regno = $aid99";
		//$stm99 = oci_parse($dbconn, $query99);
		//oci_bind_by_name($stm99, ":regno", $aid99);
		//oci_execute($stm99);
        ...
        if ($stm) {
            //echo "<script>alert('trying to logging..');</script>";
			 //echo "<script>alert($email99);</script>";
			 //echo $email99;
            //header("location: dashboard.php");
            exit;
