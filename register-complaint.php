...
if(isset($_POST['submit']))
{
    ...
    if ($imgfile != '') :
        // Get the image extension using pathinfo
        ...
        if (!in_array($extension, $allowed_extensions)) {
            echo "<script>alert('Invalid format. Only jpg / jpeg / png / gif / pdf formats are allowed');</script>";
        } else {
            // Rename the image file
        $imgnewfile = md5($imgfile . time()) . '.' . $extension;
        // Code to move the image into the directory
        ...
        $query = "INSERT INTO complaints (ComplainNumber, userId, complaintType, complaintDetails, complaintDoc) 
                  VALUES (:cnumber, :aid, :complainttype, :complaintdetails, :imgnewfile)";
        $stmt = oci_parse($dbconn, $query);
        oci_bind_by_name($stmt, ':cnumber', $cnumber);
        oci_bind_by_name($stmt, ':aid', $aid);
        oci_bind_by_name($stmt, ':complainttype', $complainttype);
        oci_bind_by_name($stmt, ':complaintdetails', $complaintdetails);
        oci_bind_by_name($stmt, ':imgnewfile', $imgnewfile);
        oci_execute($stmt);
        ...
        $query="insert into complaints(ComplainNumber,userId,complaintType,complaintDetails,complaintDoc) values(:cnumber,:aid,:complainttype,:complaintdetails,NULL)";
        $stmt = oci_parse($dbconn, $query);
        oci_bind_by_name($stmt, ':cnumber', $cnumber);
        oci_bind_by_name($stmt, ':aid', $aid);
        oci_bind_by_name($stmt, ':complainttype', $complainttype);
